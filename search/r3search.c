...
static int8_t g[MAXN][MAXPTS];   /* g[d] : partial link over positions 0..d-1 */
static int8_t ellbuf[MAXN][MAXPTS];

/* saved per-step context for recursion between steps */
typedef struct { int V, wdeg[MAXN], isfresh[MAXN], Pzero, Pmass, qmass; } ctx_t;
...
static int qterm_mask[64], qterm_n;   /* all candidate Q terms S (with bit V), ascending mask */
...
static void finish_link(void) {
    links_done++;
    int d = n - 1, npts = 1 << d;
    for (int x = 0; x < npts; x++) { int a = g[d][x]; if (a != 0 && a != 4 && a != -4) return; }
    int mv = Pmass + qmass;
    ...
    save.Pzero = Pzero; save.Pmass = Pmass; save.qmass = qmass;
    step(V + 1);
    V = save.V; memcpy(wdeg, save.wdeg, sizeof(wdeg)); memcpy(isfresh, save.isfresh, sizeof(isfresh));
    Pzero = save.Pzero; Pmass = save.Pmass; qmass = save.qmass;
    used_total -= qmass;
...
int main(int argc, char **argv) {
    if (argc >= 2 && strcmp(argv[1], "-links") == 0) {
        ...
        n = atoi(argv[1]);
        if (argc >= 3) MASSMAX = atoi(argv[2]);
    } else { fprintf(stderr, "usage: r3search N [MASSMAX] | r3search -links K M\n"); return 1; }
