...
% Compiles with pdflatex, xelatex or tectonic. The Lean listings in Appendix B contain
% mathematical Unicode characters; each is mapped below to its LaTeX equivalent, so no
% special font is required.
...
%%% The Unicode characters of the Lean listings, set in the document's own maths fonts.
\newcommand{\leansym}[1]{\ensuremath{#1}}
\newunicodechar{ℕ}{\leansym{\mathbb{N}}}
\newunicodechar{ℤ}{\leansym{\mathbb{Z}}}
\newunicodechar{ℝ}{\leansym{\mathbb{R}}}
\newunicodechar{∑}{\leansym{\textstyle\sum}}
\newunicodechar{∀}{\leansym{\forall}}
\newunicodechar{∃}{\leansym{\exists}}
\newunicodechar{¬}{\leansym{\neg}}
\newunicodechar{∧}{\leansym{\wedge}}
\newunicodechar{∨}{\leansym{\vee}}
\newunicodechar{∈}{\leansym{\in}}
\newunicodechar{∉}{\leansym{\notin}}
\newunicodechar{≠}{\leansym{\neq}}
\newunicodechar{≤}{\leansym{\leq}}
\newunicodechar{≥}{\leansym{\geq}}
\newunicodechar{→}{\leansym{\to}}
\newunicodechar{↔}{\leansym{\leftrightarrow}}
\newunicodechar{⟨}{\leansym{\langle}}
\newunicodechar{⟩}{\leansym{\rangle}}
\newunicodechar{Δ}{\leansym{\mathit{\Delta}}}
\newunicodechar{δ}{\leansym{\delta}}
\newunicodechar{²}{\leansym{^2}}
\newunicodechar{·}{\leansym{\cdot}}
\newunicodechar{∋}{\leansym{\ni}}
\newunicodechar{⊆}{\leansym{\subseteq}}
\newcommand{\leanv}[1]{\texttt{#1}}
\newcommand{\leanref}[1]{\textup{[\,\texttt{#1}\,]}}
...
%%% Status of the Lean formalisation. Flip the argument to "complete" when F_eleven is gated.
%%% Exactly two settings are intended:
%%%   \newcommand{\LeanStatus}{complete}   %  delta = 2 still building
%%%   \newcommand{\LeanStatus}{complete}     %  F(11) assembled and gated
\newcommand{\LeanStatus}{complete}
\newif\ifleandone
\makeatletter
\edef\@lsa{\LeanStatus}\def\@lsb{complete}
\ifx\@lsa\@lsb \leandonetrue \else \leandonefalse \fi
\newcommand{\leanstate}[2]{\ifleandone #1\else #2\fi}
\makeatother
\DeclareMathOperator{\Deg}{deg}

\begin{document}

\title[The Nisan--Szegedy bound is never tight]
{The Nisan--Szegedy bound on relevant variables\\ is never tight}
...
For $d = 3$ this already follows from the value $R_3 = 10$, proved by Tarannikov and Kirienko in
2000 in the language of resilient functions, and for $d \ge 8$ from Wellens' explicit bounds; the
new cases are $4 \le d \le 7$, where we obtain $R_4 \le 31$, $R_5 \le 79$, $R_6 \le 191$ and
$R_7 \le 447$. Second, we give a new, Fourier-analytic proof of $R_3 = 10$, the value attained by
the Chiarelli--Hatami--Saks function $\XiC_3$. Hence $R_1 = 1$, $R_2 = 4$, $R_3 = 10$, and
$22 \le R_4 \le 31$.
The value $R_3 = 10$ is confirmed independently by an exhaustive canonical-form search over
a finite reformulation of the problem. The finite reformulation itself has been formalised in
Lean~4 and machine-checked on the three standard axioms, with no \texttt{native\_decide}:
the twelve-variable statement $F(12)$ is proved,
\leanstate{and so is the eleven-variable statement $F(11)$.}{and the eleven-variable statement
$F(11)$ is proved in the cases $\delta = 4$ and $\delta = 0$, the case $\delta = 2$ being in
progress.}
\end{abstract}
...
The improved constants of \cite{CHS20,Wel22} are asymptotic, but the recursion behind them also
gives explicit bounds for fixed $d$. Wellens' table of bounds on the Chiarelli--Hatami--Saks
measure $W(f) = \sum_{i \in R(f)} 2^{-\deg_i f}$ \cite[Table~2]{Wel19} gives only $W \le d/2$,
which is \eqref{eq:ns}, for $d \le 7$, but $W \le 3.9375$ for $d = 8$, so that
$R_8 \le 1008 < 8 \cdot 2^7$; for $d \ge 9$ the uniform bound $R_d \le 4.416 \cdot 2^d$ is
already below $d\,2^{d-1}$. So \eqref{eq:ns} was known not to be attained for $d \ge 8$, while
for $3 \le d \le 7$ it is the best bound this line of work gives. The case $d = 3$ was, however,
settled long ago in a different literature. Multiplying by the parity $\chi_{[n]}$ turns a
Boolean function of degree $d$ with $n$ relevant variables into an $(n-d-1)$-resilient function
that depends nonlinearly on all $n$ variables (Remark~\ref{rem:resilient}), and Tarannikov and
Kirienko \cite{TK00} proved in 2000 that for $d = 3$ such a function has at most $10$ variables;
hence $R_3 = 10$. Krotov and Valyuzhenich \cite{KV24} state this in the language of degree. In
the same resilient-function language, Tarannikov, Korolev and Botev \cite{TKB01} reproved
\eqref{eq:ns} and recorded that $3 \cdot 2^{d-1} - 2$ variables are attainable.

\subsection{Results}
We settle the tightness of \eqref{eq:ns} for every $d$ by one uniform structural argument (the
cases $d = 3$ and $d \ge 8$ were known \cite{TK00,Wel19}; the cases $4 \le d \le 7$ are new), and
give a new proof of $R_3 = 10$.
...
\begin{theorem}[Tarannikov--Kirienko \cite{TK00}]\label{thm:main-r3}
...
any of them. Finally, the finite statement $F(n)$ itself has been formalised in Lean~4 and
kernel-checked on the three standard axioms: $F(12)$ is a theorem of that development, and
\leanstate{so is $F(11)$.}{so are the cases $\delta = 4$ and $\delta = 0$ of $F(11)$, with
$\delta = 2$ in progress.} Section~\ref{sec:lean} states exactly what is machine-checked and
what is not; in particular the reduction from $F(n)$ to the statement about Boolean functions
is on paper, not in Lean.

\subsection{Related work and novelty}
Nisan and Szegedy \cite{NS94} do not discuss whether \eqref{eq:ns} is tight for $d \ge 3$.
Chiarelli--Hatami--Saks \cite{CHS20} and Wellens \cite{Wel19,Wel22} beat $d\,2^{d-1}$ for large
$d$, and Wellens' Table~2 \cite{Wel19} already does so at $d = 8$ ($R_8 \le 1008$). Non-attainment
of \eqref{eq:ns} for $d \ge 8$ is therefore not new; what is new there is only the uniform
structural proof. Neither paper states an exact value of $R_3$ or $R_4$ or improves
\eqref{eq:ns} for $3 \le d \le 7$.

The exact value $R_3 = 10$ is older than all of this. It is Theorem~11 of Tarannikov and Kirienko
\cite{TK00}, in the form $p(4) = 10$, where $p(k)$ is the least integer such that every
$(n-k)$-resilient Boolean function depends nonlinearly on at most $p(k)$ variables.
Remark~\ref{rem:resilient} gives the dictionary $R_d = p(d+1)$, which is Lemma~1 of Krotov and
Valyuzhenich \cite{KV24}; they record that a Boolean function of degree at most $3$ has at most
$10$ essential variables. Tarannikov, Korolev and Botev \cite{TKB01} proved
$p(k) \le (k-1)\,2^{k-2}$, which is \eqref{eq:ns} again, without excluding equality. Our proof of
Theorem~\ref{thm:main-r3} is independent of that literature, and its finite core has been
machine-checked (Section~\ref{sec:lean}). An earlier version of this manuscript (release v1.0)
missed \cite{TK00,TKB01,KV24} and the small-degree table of \cite{Wel19}, and claimed
Theorem~\ref{thm:main-r3} and the cases $d = 3$ and $d \ge 8$ of Theorem~\ref{thm:main-ns} as new.
To the best of our knowledge, Theorem~\ref{thm:main-ns} for $4 \le d \le 7$ and
Lemma~\ref{lem:A} are new, as is the proof of Theorem~\ref{thm:main-r3} given here.

\begin{remark}[Degree and resiliency]\label{rem:resilient}
For $f\colon \{-1,1\}^n \to \{-1,1\}$ put $f' := f \cdot \chi_{[n]}$, so that
$\Fhat{f'}(S) = \Fhat{f}([n] \setminus S)$ for every $S \subseteq [n]$. By the Xiao--Massey
characterisation \cite{XM88}, $f'$ is $t$-resilient if and only if $\Fhat{f'}(S) = 0$ for all
$|S| \le t$. Hence $\deg f \le d$ if and only if $f'$ is $(n-d-1)$-resilient. Moreover $x_i$ is
irrelevant for $f$ if and only if $\Fhat{f}(T) = 0$ for all $T \ni i$, if and only if
$\Fhat{f'}(S) = 0$ for all $S \not\ni i$, if and only if $f' = x_i\, g$ with $g$ independent of
$x_i$, that is, $f'$ depends linearly on $x_i$. Therefore $R_d = p(d+1)$ for every $d$.
\end{remark}
...
Section~\ref{sec:comput} describes the computational verification and Section~\ref{sec:lean} the
formalisation in Lean~4, whose statements are reproduced in Appendix~\ref{app:leanstmt}.
...
\begin{fact}[Granularity]\label{fact:gran}
If $f \colon \cube^n \to \cube$ has $\Deg f \le d$, then every Fourier coefficient
$\Fhat{f}(S)$ is an integer multiple of $2^{1-d}$.
\end{fact}

This is standard: it is implicit in \cite{NS94} and is proved in \cite{ODbook} as the
granularity of a Boolean function of degree $d$. (We quote no exercise number.)
...
The finite statement $F(n)$ of Section~\ref{sec:finite} has been transcribed into Lean~4 and
theorems about it have been proved and kernel-checked. This section says exactly what the
transcription is, what has been proved, and --- just as important --- which part of the chain
from $F(n)$ to $R_3 = 10$ is \emph{not} machine-checked.

\subsection{The frozen finite statement}
The file \texttt{R3/Statement.lean} is frozen: it is written once, from the definition in
Section~\ref{sec:finite}, and no later file edits it. A subset $S \subseteq [n]$ is encoded as
a natural number used as a bitmask, so that $i \in S$ means \texttt{S.testBit i}, symmetric
difference is bitwise \texttt{xor} (written \texttt{\^{}\^{}\^{}}), and $|S|$ is
\texttt{card n S}, the number of set bits below $n$. A coefficient vector is a function
$N : \mathbb{N} \to \mathbb{Z}$, supported on sets of size at most $3$; its values outside
$S < 2^n$ are never read. The conditions \texttt{CondI}, \texttt{CondII}, \texttt{CondIII} are
(i), (ii), (iii) of Section~\ref{sec:finite} verbatim, \texttt{IsSol n N} is their conjunction
together with the support condition, and
\[
  \texttt{F n} \; := \; \neg\, \exists\, N, \ \texttt{IsSol n N}.
\]
Appendix~\ref{app:leanstmt} reproduces the file's definitions and the headline theorems
character for character.

Two further definitions are used in the proofs, both in terms of the frozen ones: the
\emph{mass} of a vertex, $\texttt{mass n N v} = \sum_{S \ni v} n_S^2$, which is $16\,\Inf_v$,
and \texttt{Cubic n N}, ``every set with a nonzero coefficient has exactly three elements'',
which is the hypothesis $\delta = 0$ of Appendix~\ref{app:delta0}.

\subsection{What is machine-checked}
The development is a single Lean library in the directory \texttt{lean/} of the repository of
Appendix~\ref{app:leansrc}, about $5{,}000$ lines, built with toolchain
\texttt{leanprover/lean4:v4.23.0} against the matching Mathlib. It contains no \texttt{sorry},
no \texttt{admit}, no user \texttt{axiom}, no \texttt{native\_decide}, and no
\texttt{unsafe}/\texttt{implemented\_by}/\texttt{extern} escape hatch. The gate script
\texttt{gate.sh} runs \texttt{lake build}, greps the sources for each of those constructs
(stripping backtick-quoted prose first, so that a doc comment naming one of them does not
trigger), and runs \texttt{\#print axioms} on every listed theorem, requiring that each depend
only on
\[
  \texttt{propext}, \qquad \texttt{Classical.choice}, \qquad \texttt{Quot.sound},
\]
Lean's three standard axioms. It exits $0$ only if all three checks pass, and writes its
transcript to \texttt{GATE.txt}. The check does pass, currently on $158$ listed theorems.

The headline results are:
\begin{itemize}[leftmargin=1.4em]
  \item \texttt{F\_twelve : F 12}. The twelve-variable finite statement is proved outright.
  Its proof is the formal counterpart of Theorem~\ref{thm:no12}: all masses are $4$, all terms
  cubic with coefficients $\pm1$, every link is a $4$-cycle, the octahedral closure produces a
  six-element vertex set closed under the support, and the disjoint-sum contradiction closes it.
  \item \texttt{eleven\_delta\_four}: no solution on eleven variables has all masses $4$. This
  is the case $\delta = 4$ of Appendix~\ref{app:delta4}.
  \item \texttt{eleven\_delta\_zero}: no solution on eleven variables has all its support sets
  of size $3$. This is the case $\delta = 0$, and it is the argument printed in
  Appendix~\ref{app:delta0}, step for step; every lemma there carries the name of the theorem
  that certifies it.
  \item \leanstate{\texttt{eleven\_delta\_two}: the case $\delta = 2$, one vertex of mass $6$
  and the rest of mass $4$.}{the case $\delta = 2$ (one vertex of mass $6$) is in progress; two
  of its three branches are gated and the one-linear-term branch is open.}
\end{itemize}
\ifleandone
The three cases are assembled into
\[
  \texttt{F\_eleven : F 11},
\]
so that both finite statements needed for $R_3 = 10$ are machine-checked.
\else
\todo{F\_eleven: fill in after gate --- state \texttt{F\_eleven : F 11}, its one-line proof by
the bookkeeping trichotomy $\delta \in \{4,2,0\}$ from \texttt{bookkeeping}, and add it to
Appendix~\ref{app:leanstmt}; then set \texttt{\textbackslash LeanStatus} to
\texttt{complete}.}
\fi

The supporting theorems cover the basic identities ($f_N^2 = 16$ in integer form, the pointwise
condition (E1), divisibility and lower bounds on the mass, $\sum_v m_v \le 48$),
Lemma~\ref{lem:masses} (mass in $\{4,6,8\}$, and at most two vertices of mass other than $4$),
the bookkeeping identity \eqref{eq:bookkeeping} in the form $e + \delta = 48 - 4n$, the link
classification of Lemma~\ref{lem:links}(a), the $4$-cycle structure of a mass-$4$ link, the
sixth-vertex and three-faces lemmas used for closure, and the octahedron kill of
Lemma~\ref{lem:octakill}.

\subsection{What is not machine-checked}
Three things.

First, the reduction from $F(n)$ to the statement about Boolean functions is on paper. It is the
granularity argument of Section~\ref{sec:finite}: if $f \colon \cube^n \to \cube$ has degree at
most $3$ then $N := 4\Fhat{f}$ is an integer vector satisfying (i)--(iii), so $F(n)$ implies
that no degree-$3$ Boolean function has $n$ relevant variables. Fact~\ref{fact:gran} (integer
granularity), Parseval, and the identity $\chi_S\chi_T = \chi_{S \sd T}$ are the only
ingredients, and none of them is formalised here. What Lean certifies is a statement about
integer vectors, not about Boolean functions.

Second, Theorem~\ref{thm:main-ns} --- the general-$d$ result --- is not formalised at all, and
neither is the exhaustive search of Section~\ref{sec:comput}: the latter is far beyond kernel
\texttt{decide}, and nothing in the Lean development depends on it.

Third, \leanstate{nothing else: with \texttt{F\_twelve} and \texttt{F\_eleven} both proved, the
whole of Part~I and Appendix~\ref{app:cases} is covered in its finite form.}{the case
$\delta = 2$ of $F(11)$ is not yet complete, so $F(11)$ itself is not yet a Lean theorem; the
paper proof of that case (Appendix~\ref{app:delta2}) stands on its own and has been refereed.}

We also record what the formalisation is \emph{not} evidence for. A kernel check certifies that
the proofs are correct proofs of the Lean statements; that those statements say what
Section~\ref{sec:finite} says is a matter of reading \texttt{Statement.lean}, which is why it is
frozen and why it is reproduced verbatim in Appendix~\ref{app:leanstmt}.
...
\[
  R_1 = 1, \qquad R_2 = 4, \qquad R_3 = 10, \qquad 22 \le R_4 \le 31,
\]
where $R_3 = 10$ is due to \cite{TK00}, $R_4 \ge 22 = 3 \cdot 2^3 - 2$ comes from $\XiC_4$ and $R_4 \le 31 = 4 \cdot 2^3 - 1$ from
...
symmetries. This agrees with the resilient-function classification used by Krotov and
Valyuzhenich, who also note that the degree-$3$ function with $10$ essential variables is unique
\cite{KV24}; that classification is computational too. We have no proof of uniqueness by hand.
...
fourth four-set link shape (L4) of Lemma~\ref{lem:links}(a). The topology-free treatment of the
case $\delta = 0$ in Appendix~\ref{app:delta0} is the argument that was formalised in Lean; it
replaces an earlier argument by Euler characteristics, and it too was refereed.
...
Now $f = \frac14\sum_S n_S \chi_S$ is supported on triples, with $\sum_S n_S^2 = 16$. This is
the case that has been formalised in Lean~4 (Section~\ref{sec:lean}), and the argument given
here is the formalised one: it uses no topology, no classification of the link shapes and no
Euler characteristic. The name of the Lean theorem certifying each step is given in brackets;
all of them live in \texttt{R3/DeltaZero.lean}, and the case as a whole is
\texttt{eleven\_delta\_zero}.

\subsubsection*{Setting and standing facts}
Call a vertex \emph{ordinary} if its mass is $4$ and \emph{exceptional} otherwise. Write
$\{x,y,z\}$ for a \emph{support triple}, that is, a set $S$ of size $3$ with $n_S \ne 0$. Two
facts about an ordinary vertex $u$ are used throughout, both from Lemma~\ref{lem:links}(a):
\begin{itemize}[leftmargin=1.4em]
  \item[(M1)] $u$ lies in exactly four support sets \leanref{mass\_four\_card}, and the
  coefficient on each of them is $\pm1$ \leanref{mass\_four\_pm}. So four distinct support sets
  exhibited at $u$ are all of them \leanref{supp\_eq\_quad}, and there is no fifth
  \leanref{no\_fifth\_supp}.
  \item[(M2)] If $S_1, S_2, S_3, S_4$ are the four support sets at $u$ then
  $S_1 \sd S_2 \sd S_3 \sd S_4 = \emptyset$ and $n_{S_1} n_{S_2} n_{S_3} n_{S_4} = +1$
  \leanref{mass\_four}. The second half is the \emph{sign relation} at $u$; it is the only place
  the signs enter the argument, and it is what makes the route work.
\end{itemize}
Since every support set is a triple, (M1) says that the link of an ordinary vertex $u$ is a
$4$-cycle of pairs $p$--$q$--$r$--$s$--$p$: the support triples at $u$ are exactly $u$ together
with an edge of that cycle \leanref{LinkIs}. In particular a pair through an ordinary vertex
lies in exactly $0$ or $2$ support triples \leanref{pair\_second, pair\_not\_three}.

\subsubsection*{Step 1: the degree sequence}
With all terms cubic, $\sum_v m_v = 3 \cdot 16 = 48$ \leanref{cubic\_sum\_mass}, so the excess
is $\sum_v (m_v - 4) = 48 - 44 = 4$. By Lemma~\ref{lem:masses} every mass is $4$, $6$ or $8$
and at most two vertices are exceptional, so the degree sequence is $(8, 4^{10})$ or
$(6,6,4^9)$ \leanref{eleven\_degree\_split}. No coefficient $|n_S| = 2$ occurs, by
Lemma~\ref{lem:links}(d). \emph{At most two vertices are exceptional} is the only form in which
the degree sequence is used below.

\subsubsection*{Step 2: the half-octahedron}

\begin{lemma}[Half-closure; \texttt{octa\_half}]\label{lem:octahalf}
Let $\{v,a,b\}$ be a support triple with $a$ and $b$ ordinary and $v, a, b$ distinct. Then there
are vertices $a', b', y$ with
\[
  \lk(a) = b\text{--}v\text{--}b'\text{--}y, \qquad \lk(b) = a\text{--}v\text{--}a'\text{--}y,
\]
with the \emph{same} $y$; moreover $y \notin \{v,a,b\}$, and each of $a', b'$ is distinct from
$a, b, v$ and $y$. The mass of $v$ is not used, so $v$ may be exceptional.
\end{lemma}

\begin{proof}
Since $a$ is ordinary, the pair $\{a,v\}$ lies in a second support triple $\{a,v,b'\}$ with
$b' \notin \{a,b,v\}$ \leanref{second\_v\_triple}. The link of $a$ is a $4$-cycle containing the
two edges $\{v,b\}$ and $\{v,b'\}$ at $v$, hence is $b$--$v$--$b'$--$y_1$ for a sixth vertex
$y_1 \notin \{a,v,b,b'\}$ \leanref{link\_sixth}. Symmetrically $\lk(b) = a$--$v$--$a'$--$y_2$.
The edge $\{y_1,b\}$ of $\lk(a)$ gives the support triple $\{a,b,y_1\}$ and the edge
$\{y_2,a\}$ of $\lk(b)$ gives $\{a,b,y_2\}$. If $y_1 \ne y_2$ then the pair $\{a,b\}$ would lie
in the three distinct support triples $\{a,b,v\}, \{a,b,y_1\}, \{a,b,y_2\}$, impossible at the
ordinary vertex $a$ \leanref{pair\_not\_three}. So $y_1 = y_2 =: y$.
\end{proof}

Lemma~\ref{lem:octahalf} says \emph{nothing} about $a'$ versus $b'$: it does not assert
$a' \ne b'$. That distinctness is proved separately each time it is needed, from the fact that
the $4$-cycle link of the ordinary vertex $y$ contains no triangle \leanref{no\_triangle\_at};
see Step 4.

\subsubsection*{Step 3: the kill}

Say that $v, a, b, a', b', y$ \emph{carry the four octahedron links} if the six vertices are
pairwise distinct \leanref{Dist6} and
\begin{equation}\label{eq:octa4}
  \lk(a) = b\text{--}v\text{--}b'\text{--}y, \quad \lk(b) = a\text{--}v\text{--}a'\text{--}y,
  \quad \lk(y) = b\text{--}a\text{--}b'\text{--}a', \quad
  \lk(a') = b\text{--}y\text{--}b'\text{--}v
\end{equation}
\leanref{Octa4}. Geometrically this is an octahedron with apex $v$ and antipode $y$; the fifth
link, that of $b'$, is deliberately omitted, and is never used.

\begin{lemma}[The kill; \texttt{octa\_kill4}]\label{lem:octakill}
If $v, a, b, a', b', y$ carry the four links \eqref{eq:octa4} and the four vertices $a$, $b$,
$a'$ and $y$ are ordinary, then $f$ is not Boolean. The mass of $v$, the mass of $b'$ and the
link of $b'$ are not used. The mirror statement, with $a \leftrightarrow b$ and
$a' \leftrightarrow b'$ exchanged, needs mass $4$ at $a, b, b', y$ and the link of $b'$ in place
of that of $a'$ \leanref{octa\_kill4'}.
\end{lemma}

\begin{proof}
The four edges of $\lk(y)$ give the support triples
$\{y,a,b\}, \{y,a,b'\}, \{y,a',b'\}, \{y,a',b\}$, and $\lk(a), \lk(b), \lk(a')$ give
$\{v,a,b\}, \{v,a,b'\}, \{v,a',b'\}, \{v,a',b\}$. The first four are distinct, so by (M1) they
exhaust the support of $y$ \leanref{supp\_eq\_quad}.

Now use $f^2 \equiv 1$ at the pair $U = \{v,y\}$: the coefficient of $\chi_U$ in $f^2$ is
$\sum_{S} n_S n_{S \sd U} = 0$, the sum over all $S$. Since $y \in U$, the involution
$S \mapsto S \sd U$ toggles membership of $y$ and so pairs each set containing $y$ with a set
not containing $y$; the two halves of the sum are equal, and therefore
\begin{equation}\label{eq:corr}
  \sum_{S \ni y,\; n_S \ne 0} n_S\, n_{S \sd \{v,y\}} = 0
\end{equation}
\leanref{condII\_corr, corr\_bit\_half, corr\_supp}. Since $a, b, a', b'$ differ from $v$ and
from $y$, flipping the apex sends $\{y,p,q\}$ to $\{v,p,q\}$ \leanref{tri\_xor\_pair}, so the
four terms of \eqref{eq:corr} are exactly
\[
  \eps_1 = n_{vab}\,n_{yab}, \quad \eps_2 = n_{vab'}\,n_{yab'}, \quad
  \eps_3 = n_{va'b'}\,n_{ya'b'}, \quad \eps_4 = n_{va'b}\,n_{ya'b},
\]
and $\eps_1 + \eps_2 + \eps_3 + \eps_4 = 0$. Each of the eight coefficients is $\pm1$ by (M1) at
the ordinary vertices $a$, $a'$ and $y$, so each $\eps_i = \pm1$ \leanref{pm\_mul}.

Finally the sign relation (M2) ties the $\eps_i$ together. The four support sets at $a$ are
$\{v,a,b\}, \{v,a,b'\}, \{y,a,b'\}, \{y,a,b\}$ --- distinct, hence all of them by (M1) --- so
their coefficient product is $+1$, i.e.\ $\eps_1\eps_2 = 1$. The four at $b$ are
$\{v,a,b\}, \{v,a',b\}, \{y,a',b\}, \{y,a,b\}$, giving $\eps_1\eps_4 = 1$. The four at $a'$ are
$\{v,a',b'\}, \{v,a',b\}, \{y,a',b\}, \{y,a',b'\}$, giving $\eps_3\eps_4 = 1$. Hence
$\eps_2 = \eps_4 = \eps_3 = \eps_1$ and the sum is $4\eps_1 = \pm4 \ne 0$
\leanref{eps\_kill}.
\end{proof}

This is the entire sign content of the case. Everything that follows exists only to produce a
configuration to which Lemma~\ref{lem:octakill} applies.

\subsubsection*{Step 4: full closure at a lone exceptional vertex}

\begin{lemma}[Closure; \texttt{octa\_eight}]\label{lem:octaeight}
Let $\{v,a,b\}$ be a support triple and suppose every vertex other than $v$ is ordinary. Then
the vertices $a', b', y$ of Lemma~\ref{lem:octahalf} are pairwise distinct and distinct from
$v, a, b$, and all five links other than that of $v$ are the octahedron links: \eqref{eq:octa4}
holds, together with $\lk(b') = v$--$a$--$y$--$a'$.
\end{lemma}

\begin{proof}
Apply Lemma~\ref{lem:octahalf}; its two links give the support triples
$\{a,v,b'\}, \{a,b',y\}, \{a,y,b\}, \{b,v,a'\}, \{b,a',y\}$. If $a' = b'$ then $\lk(y)$ would
contain the three pairs $\{a',a\}, \{a',b\}, \{a,b\}$, a triangle, which the $4$-cycle link of
the ordinary vertex $y$ cannot contain \leanref{no\_triangle\_at}; hence $a' \ne b'$. Three
faces at an ordinary vertex determine its $4$-cycle link \leanref{link\_of\_three\_faces}: from
$\{y,a,b\}, \{y,a,b'\}, \{y,b,a'\}$ we get $\lk(y) = b$--$a$--$b'$--$a'$, which yields the
triple $\{y,b',a'\}$; then from $\{a',y,b\}, \{a',y,b'\}, \{a',b,v\}$ we get
$\lk(a') = b$--$y$--$b'$--$v$, which yields $\{a',b',v\}$; and from
$\{b',a,v\}, \{b',a,y\}, \{b',v,a'\}$ we get $\lk(b') = v$--$a$--$y$--$a'$.
\end{proof}

\begin{proposition}[\texttt{eleven\_delta\_zero\_eight}]\label{prop:lone}
If at most one vertex of a homogeneous cubic solution on $11$ variables is exceptional, there is
no solution.
\end{proposition}

\begin{proof}
Let $v$ be the possible exception. By relevance $v$ lies in some support set, which is a triple
$\{v,a,b\}$ with $a, b$ distinct and different from $v$. Both are ordinary, so
Lemma~\ref{lem:octaeight} closes the octahedron on $v, a, b, a', b', y$; the four vertices
$a, b, a', y$ all differ from $v$ and are therefore ordinary, and Lemma~\ref{lem:octakill}
applies.
\end{proof}

\begin{remark}\label{rem:no8}
Proposition~\ref{prop:lone} disposes of the sub-case $(8, 4^{10})$, but it never uses the value
$8$, nor indeed any property of $m_v$: the only hypothesis is that \emph{at most one vertex is
exceptional}. So the $(8, 4^{10})$ sub-case is not a mass-$8$ argument at all. Moreover
Steps~2--6 are independent of $n$: each is a statement about an arbitrary number of variables
with at most two exceptional vertices. Only Step~1 (the degree split) and Step~7 (the weight
count $6 + 6 < 16$) are arithmetic about $n = 11$.
\end{remark}

Combining Step~1 with Proposition~\ref{prop:lone}: a homogeneous cubic solution on eleven
variables has degree sequence $(6,6,4^9)$ \leanref{eleven\_delta\_zero\_reduce}. From here on
$v$ and $w$ are the two exceptional vertices, $v \ne w$, and every other vertex is ordinary.
No hypothesis on the values $m_v, m_w$ is used before Step~7.

\subsubsection*{Step 5: the apex of a triple at an exceptional vertex}

\begin{lemma}[\texttt{apex\_other}]\label{lem:apexother}
Let $\{v,a,b\}$ be a support triple with $a$ and $b$ ordinary. Then the apex $y$ supplied by
Lemma~\ref{lem:octahalf} is the \emph{other} exceptional vertex $w$:
\[
  \lk(a) = b\text{--}v\text{--}b'\text{--}w, \qquad \lk(b) = a\text{--}v\text{--}a'\text{--}w,
\]
with $a', b' \notin \{a,b,v,w\}$.
\end{lemma}

\begin{proof}
Apply Lemma~\ref{lem:octahalf}, obtaining $a', b', y$, and suppose $y \notin \{v,w\}$. Then $y$
is ordinary, so exactly as in Lemma~\ref{lem:octaeight} we get $a' \ne b'$ (else $\lk(y)$
contains a triangle) and $\lk(y) = b$--$a$--$b'$--$a'$ is forced by the three faces
$\{y,a,b\}, \{y,a,b'\}, \{y,b,a'\}$. Two cases.
\begin{itemize}[leftmargin=1.4em]
  \item If $a' \ne w$ then $a'$ is ordinary (it is not $v$ either) and its link is forced to be
  $b$--$y$--$b'$--$v$ by the faces $\{a',y,b\}, \{a',y,b'\}, \{a',b,v\}$. Now
  $v, a, b, a', b', y$ carry the four links \eqref{eq:octa4} with $a, b, a', y$ all ordinary, and
  Lemma~\ref{lem:octakill} applies.
  \item If $a' = w$ then $b'$ is ordinary ($b'$ is neither $v$ nor $a' = w$) and its link is
  forced to be $a$--$y$--$a'$--$v$ by the faces $\{b',y,a\}, \{b',y,a'\}, \{b',a,v\}$. This is
  the mirror configuration, killed by the mirror of Lemma~\ref{lem:octakill}, which needs mass
  $4$ at $a, b, b'$ and $y$.
\end{itemize}
Either way $f$ is not Boolean. So $y \in \{v,w\}$, and $y \ne v$ by Lemma~\ref{lem:octahalf};
hence $y = w$.
\end{proof}

\begin{lemma}[\texttt{no\_exc\_pair}]\label{lem:noexcpair}
No support triple contains both $v$ and $w$.
\end{lemma}

\begin{proof}
Suppose $\{u,v,w\}$ is a support triple. Then $u \notin \{v,w\}$, so $u$ is ordinary and the
pair $\{u,v\}$ lies in a second support triple $\{u,v,d\}$ with $d \ne w$ and $d \ne u$
\leanref{pair\_second}; also $d \ne v$, the three vertices of a support triple being distinct
\leanref{tri\_distinct}. So $d$ is ordinary, and Lemma~\ref{lem:apexother} applied to the triple
$\{v,u,d\}$ gives $\lk(u) = d$--$v$--$b'$--$w$ for some $b' \notin \{d,u,v,w\}$. In that
$4$-cycle the two neighbours of $v$ are $d$ and $b'$, so the only support triples containing the
pair $\{u,v\}$ are $\{u,v,d\}$ and $\{u,v,b'\}$ \leanref{link\_v\_nbrs}. But $\{u,v,w\}$ is such
a triple and $w \notin \{d,b'\}$.
\end{proof}

\subsubsection*{Step 6: killing every triple of ordinary vertices}

\begin{lemma}[Generic apex; \texttt{apex\_gen}]\label{lem:apexgen}
Let $\{x,a,b\}$ be any support triple whose ends $a$ and $b$ are ordinary; the third vertex $x$
is arbitrary, ordinary or exceptional. Then at least one of
\[
  \{v,a,b\}, \qquad \{w,a,b\}, \qquad \{v,b,x\}, \qquad \{w,b,x\}
\]
is a support triple.
\end{lemma}

\begin{proof}
Apply Lemma~\ref{lem:octahalf} at $\{x,a,b\}$, giving $a', b', y$ with
$\lk(a) = b$--$x$--$b'$--$y$ and $\lk(b) = a$--$x$--$a'$--$y$. Reading $\lk(b)$ gives the
support triple $\{b,x,a'\}$ and reading $\lk(a)$ gives $\{a,y,b\}$. If $a' \in \{v,w\}$ the
third or fourth alternative holds; if $y \in \{v,w\}$ the first or second does. So assume $y$
and $a'$ are both ordinary. Then, exactly as in Lemma~\ref{lem:apexother}, $a' \ne b'$,
$\lk(y) = b$--$a$--$b'$--$a'$ and $\lk(a') = b$--$y$--$b'$--$x$ are forced, so
$x, a, b, a', b', y$ carry the four links \eqref{eq:octa4} with $a, b, a', y$ ordinary, and
Lemma~\ref{lem:octakill} applies.
\end{proof}

\begin{lemma}[\texttt{kill\_pair}]\label{lem:killpair}
Let $e \in \{v,w\}$, let $s$ and $t$ be ordinary and let $\{e,s,t\}$ be a support triple. Then
no ordinary vertex $u$ completes the pair $\{s,t\}$: $\{s,t,u\}$ is not a support triple.
\end{lemma}

\begin{proof}
Write $e_1 = e$ and let $e_2$ be the other of $v, w$; here it matters that $v \ne w$, so that
$e_1$ and $e_2$ are genuinely two vertices. Lemma~\ref{lem:apexother} applied at $\{e_1,s,t\}$
gives $\lk(s) = t$--$e_1$--$t'$--$e_2$ for some $t' \notin \{s,t,e_1,e_2\}$. In this $4$-cycle
the two neighbours of $t$ are $e_1$ and $e_2$, so the only support triples containing the pair
$\{s,t\}$ are $\{s,t,e_1\}$ and $\{s,t,e_2\}$ \leanref{link\_v\_nbrs}. An ordinary $u$ is
neither.
\end{proof}

\begin{lemma}[No free triple; \texttt{kill\_free\_triple}]\label{lem:killfree}
Assume $v \ne w$. Then no support triple has all three vertices ordinary.
\end{lemma}

\begin{proof}
Let $\{p,q,r\}$ be such a triple and apply Lemma~\ref{lem:apexgen} with $x = p$, $a = q$,
$b = r$. In the first two cases $\{v,q,r\}$ or $\{w,q,r\}$ is a support triple with $q, r$
ordinary, while $\{q,r,p\}$ is a support triple with $p$ ordinary, contradicting
Lemma~\ref{lem:killpair} with $(s,t,u) = (q,r,p)$. In the last two cases $\{v,r,p\}$ or
$\{w,r,p\}$ is a support triple with $r, p$ ordinary, while $\{r,p,q\}$ is a support triple with
$q$ ordinary, contradicting Lemma~\ref{lem:killpair} with $(s,t,u) = (r,p,q)$.
\end{proof}

\subsubsection*{Step 7: the endgame}

\begin{lemma}[\texttt{exists\_free\_set}]\label{lem:freeset}
If $m_v = m_w = 6$ and no support set contains both $v$ and $w$, then some support set contains
neither.
\end{lemma}

\begin{proof}
If every support set met $\{v,w\}$ then, the two supports being disjoint, they would partition
the whole support, giving $16 = \sum_S n_S^2 = m_v + m_w = 12$. (Only $\sum_S n_S^2 = 16$ is
used here; homogeneity is not.)
\end{proof}

\begin{proof}[Proof that $\delta = 0$ is impossible]
By Step~1 and Proposition~\ref{prop:lone} the degree sequence is $(6,6,4^9)$; let $v \ne w$ be
the two vertices of mass $6$, every other vertex being ordinary. A support set containing both
$v$ and $w$ would be a triple $\{u,v,w\}$ with $u$ ordinary, which Lemma~\ref{lem:noexcpair}
forbids, so the supports of $v$ and of $w$ are disjoint. By Lemma~\ref{lem:freeset} some support
set contains neither $v$ nor $w$; it is a triple $\{p,q,r\}$, and $p, q, r$ are all ordinary.
Lemma~\ref{lem:killfree} forbids it. \leanref{eleven\_delta\_zero}
\end{proof}

\begin{remark}
The route never classifies the link of an exceptional vertex, so the shapes $C_8$,
$C_3 \cup C_5$, $C_4 \cup C_4$ and the bowtie are never named. An independent seeded search
reports that in the $(8,4^{10})$ branch only $C_4 \cup C_4$ admits a combinatorial completion,
with exactly $630$ of them, all of them two octahedra glued at $v$, and that nothing
combinatorial excludes them: they die only to a sign argument. Lemma~\ref{lem:octakill} is that
sign argument, localised to a single octahedron and to the single pair $U = \{v,y\}$. The $630$
configurations are therefore exactly the ones it kills.
\end{remark}
...
\section{The Lean statements}\label{app:leanstmt}
\sloppy
The following are copied from the sources, character for character; only statements are shown,
never proofs. Appendix~\ref{app:leansrc} says where the proofs are.

\subsection{The frozen finite statement}
All of \texttt{R3/Statement.lean} except its header comment and its \texttt{import Mathlib}:

\begin{Verbatim}[fontsize=\footnotesize,formatcom=\ttfamily,samepage=false]
...
The bitmask encoding is the whole of the translation: \texttt{S.testBit i} is $i \in S$,
\texttt{\^{}\^{}\^{}} is $\sd$, and \texttt{range (2 \^{} n)} enumerates the subsets of $[n]$.
Note that \texttt{CondII} sums over \emph{ordered} pairs, as in Section~\ref{sec:finite}.

\subsection{The auxiliary definitions}
From \texttt{R3/Basic.lean} and \texttt{R3/Octahedron.lean}:
...
Here \texttt{tri v x y} is the bitmask of $\{v,x,y\}$. \texttt{Cubic 11 N} is the hypothesis
$\delta = 0$; \texttt{mass n N v} is $m_v = 16\,\Inf_v(f)$.

\subsection{The headline theorems}
From \texttt{R3/Final.lean}, \texttt{R3/DeltaFour.lean} and \texttt{R3/DeltaZero.lean}:

\begin{Verbatim}[fontsize=\footnotesize,formatcom=\ttfamily,samepage=false]
theorem F_twelve : F 12

theorem eleven_delta_four {N : ℕ → ℤ} (hsol : IsSol 11 N)
    (hm : ∀ v, v < 11 → mass 11 N v = 4) : False

theorem eleven_delta_zero {N : ℕ → ℤ} (hsol : IsSol 11 N) (hcub : Cubic 11 N) : False
\end{Verbatim}

\ifleandone
The case $\delta = 2$ and the assembly are
\begin{Verbatim}[fontsize=\footnotesize,formatcom=\ttfamily,samepage=false]
theorem eleven_delta_two {N : ℕ → ℤ} (hsol : IsSol 11 N) {v : ℕ} (hv : v < 11)
    (h6 : mass 11 N v = 6) (hrest : ∀ w, w < 11 → w ≠ v → mass 11 N w = 4) : False

theorem F_eleven : F 11
\end{Verbatim}
\else
The case $\delta = 2$ is in progress; its statement is
\begin{Verbatim}[fontsize=\footnotesize,formatcom=\ttfamily,samepage=false]
theorem eleven_delta_two {N : ℕ → ℤ} (hsol : IsSol 11 N) {v : ℕ} (hv : v < 11)
    (h6 : mass 11 N v = 6) (hrest : ∀ w, w < 11 → w ≠ v → mass 11 N w = 4) : False
\end{Verbatim}
and \todo{F\_eleven: fill in after gate --- paste the verbatim statement of
\texttt{F\_eleven : F 11} here.}
\fi

The octahedron kill of Lemma~\ref{lem:octakill}, the one place the signs are used, is stated
against the four links \eqref{eq:octa4}:
...
theorem octa_kill4 {n : ℕ} {N : ℕ → ℤ} (hsol : IsSol n N)
    ...
    (h4y : mass n N y = 4)
    (hD : Dist6 v a b a' b' y) (hO : Octa4 n N v a b a' b' y) : False
\end{Verbatim}

Two features of this statement are worth reading off, because they are exactly the claims made
in Appendix~\ref{app:delta0}: there is no hypothesis \texttt{mass n N v} and no hypothesis
\texttt{mass n N b'}, and \texttt{Octa4} names four links, not five. The theorem also does not
take \texttt{Cubic}. \texttt{Dist6} is the conjunction of the fifteen distinctness facts among
$v, a, b, a', b', y$.

\subsection{The axiom certificate}
The \texttt{\#print axioms} output for the headline theorems reads

\begin{Verbatim}[fontsize=\scriptsize,formatcom=\ttfamily,samepage=false]
'R3.F_twelve' depends on axioms: [propext, Classical.choice, Quot.sound]
'R3.eleven_delta_four' depends on axioms: [propext, Classical.choice, Quot.sound]
'R3.eleven_delta_zero' depends on axioms: [propext, Classical.choice, Quot.sound]
\end{Verbatim}

and likewise for each of the other listed theorems. No line of the gate's output names any
other axiom. The list of theorems the gate audits grows as each case is merged into the
top-level module; a theorem is quoted here only once its own file has been built and its
axioms printed.

\section{The Lean development}\label{app:leansrc}
The formal development is about $5{,}000$ lines, too long to print here. It is the directory
\texttt{lean/} of the repository
\begin{center}
  \url{https://github.com/moffatstudio/relevant-variables-degree}
\end{center}
and is also the ancillary material of this submission. Its layout is:

\begin{itemize}[leftmargin=1.4em]
  \item \texttt{R3/Statement.lean} --- the frozen finite statement $F(n)$, reproduced in full in
  Appendix~\ref{app:leanstmt}. Nothing else in the development modifies it.
  \item \texttt{R3/Basic.lean}, \texttt{R3/Mass.lean}, \texttt{R3/Link.lean} --- masses,
  supports, the pointwise condition (E1), Lemma~\ref{lem:masses} and the link classification of
  Lemma~\ref{lem:links}(a).
  \item \texttt{R3/Cycle.lean}, \texttt{R3/Octahedron.lean}, \texttt{R3/LinkTypes.lean},
  \texttt{R3/LinkSix.lean} --- $4$-cycle links, \texttt{Edge}, \texttt{LinkIs}, the sixth-vertex
  and three-faces lemmas, and the closure machinery.
  \item \texttt{R3/Twelve.lean}, \texttt{R3/Final.lean} --- Theorem~\ref{thm:no12} in finite
  form, ending in \texttt{F\_twelve}.
  \item \texttt{R3/DeltaFour.lean}, \texttt{R3/DeltaTwo.lean}, \texttt{R3/DeltaZero.lean} ---
  the three cases of $F(11)$; \texttt{DeltaZero.lean} is the file whose argument is printed in
  Appendix~\ref{app:delta0}.
  \item \texttt{gate.sh} --- the gate described in Section~\ref{sec:lean}; it writes
  \texttt{GATE.txt} and \texttt{AXIOMS.txt} and exits nonzero on any failure. The same gate runs
  in continuous integration on a fresh clone, and the repository carries its badge.
\end{itemize}

From a fresh clone,
\[
  \texttt{lake exe cache get \&\& lake build \&\& bash gate.sh}
\]
reproduces the check. The toolchain is pinned by \texttt{lean-toolchain}
(\texttt{leanprover/lean4:v4.23.0}) and the Mathlib revision by \texttt{lake-manifest.json}.
...
\bibitem{KV24}
D.~S.~Krotov and A.~A.~Valyuzhenich,
\emph{On degree-$3$ and $(n-4)$-correlation-immune perfect colorings of $n$-cubes},
Discrete Math. \textbf{347} (2024), no.~10, 114138.
\texttt{arXiv:2311.05566}.
...
\bibitem{TK00}
Yu.~Tarannikov and D.~Kirienko,
\emph{Spectral analysis of high order correlation immune functions},
IACR Cryptology ePrint Archive, Report 2000/050 (2000).

\bibitem{TKB01}
Yu.~Tarannikov, P.~Korolev and A.~Botev,
\emph{Autocorrelation coefficients and correlation immunity of Boolean functions},
in: Advances in Cryptology -- ASIACRYPT 2001, Lecture Notes in Comput. Sci. \textbf{2248},
Springer, 2001, pp.~460--479.

\bibitem{Wel19}
J.~Wellens,
\emph{A tighter bound on the number of relevant variables in a bounded degree Boolean
function},
preprint, \texttt{arXiv:1903.08214} (2019).
...
\emph{A spectral characterization of correlation-immune combining functions},
IEEE Trans. Inform. Theory \textbf{34} (1988), 569--571.
